...
function img({ alias = {}, filename } = {}) {
  ...
  tokenizers.imgSyntax = tokenizeImgSyntax
  methods.splice(methods.indexOf('linkReference'), 0, 'imgSyntax')
  tokenizeImgSyntax.locator = locator
  ...
  let messages = []
  function tokenizeImgSyntax(eat, value, silent) {
    let now
    let $1
    let $2
    ...
    if (regex.test(value)) {
      ...
      if (sameList.length > 1) {
        messages.push(['多个同可能匹配的图片集: ' + JSON.stringify(permalink) + ' [' + sameList.join(', ') + ']', now])
        return
      }

      // console.error('matched', $2, title, permalink, linkedFilename)
      if (linkedFilename) {
        return eat($1)({
          type: 'image',
          title: '',
          url: nps.relative(nps.dirname(filename), linkedFilename) + hash,
          alt: title || matchedPermaLink,
        })
      }
      else {
